...
export default function SoulChat({
  ...
  return (
    <div
      className={`flex flex-col overflow-hidden rounded-2xl border border-[#dfe0dc] bg-white text-[#20221f] shadow-[0_12px_36px_rgba(0,0,0,.08)] ${
        fill ? "h-full min-h-[420px]" : compact ? "h-[560px]" : "h-[650px]"
      }`}
    >
      <div className="flex items-center justify-between border-b border-[#ecece9] px-5 py-4">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 truncate text-xs text-[#7b7f78]">{soul.personality.role}</p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button
            onClick={restart}
            className="rounded-lg p-2 text-[#777b74] hover:bg-[#f2f3f0] hover:text-[#252824]"
      ...
      <div ref={scrollRef} className="flex-1 space-y-5 overflow-y-auto px-5 py-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {message.citations.map((citation, index) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-1.5 rounded-lg border border-[#dfe0dc] bg-[#fafaf8] px-2.5 py-1.5 text-xs text-[#646861] hover:border-[#bdc0b9] hover:text-[#20221f]"
                      >
        ...
        {sending && (
          <div className="flex items-center gap-2 text-sm text-[#777b74]">
      ...
      <div className="border-t border-[#ecece9] p-4">
        {messages.length === 1 && (
          <div className="mb-3 flex gap-2 overflow-x-auto pb-1">
            {["What do you offer?", "How does it work?", "Which option fits me?"].map((prompt) => (
              <button
                ...
                className="shrink-0 rounded-full border border-[#dfe0dc] bg-white px-3 py-1.5 text-xs text-[#676b64] hover:bg-[#f4f5f2]"
              >
                {prompt}
              </button>
            ))}
        ...
        <div className="flex items-end gap-2 rounded-xl border border-[#d9dbd6] bg-[#fafaf8] p-2 focus-within:border-[#aeb4a8] focus-within:bg-white">
          <button
            onClick={toggleListening}
            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${listening ? "bg-[#a84c3e] text-white" : "text-[#747870] hover:bg-[#eceee9]"}`}
          ...
          <textarea
            ...
            className="max-h-28 min-h-9 flex-1 resize-none bg-transparent px-1 py-2 text-sm outline-none placeholder:text-[#9a9d97]"
          />
          <button
        ...
        <p className="mt-2 text-center text-xs text-[#969993]">
