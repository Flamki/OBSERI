/* eslint-disable */
// Adapted from Supertone Inc.'s official browser example.
// Copyright (c) 2025 Supertone Inc. Licensed under MIT; see THIRD_PARTY_NOTICES.md.
...
export class UnicodeProcessor {
    ...
    preprocessText(text, lang) {
        ...
        if (!/[.!?;:,'\"')\]}â€¦ã€‚ã€ã€ã€‘ã€‰ã€‹â€ºÂ»]$/.test(text)) {
            text += '.';
        }
...
export async function loadVoiceStyle(voiceStylePaths, verbose = false) {
    ...
    // Fill in the data
    for (let i = 0; i < bsz; i++) {
        const voiceStyle = i === 0
            ? firstStyle
            : await (await fetch(voiceStylePaths[i])).json();
